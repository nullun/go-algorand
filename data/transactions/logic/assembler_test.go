...
//
// The source uses #pragma typetrack false so the assembler accepts the
// conflicting types during initial assembly.  The disassembly is then
// reassembled with type tracking on, exercising the label-widening fix.
...
retsub`
	...
	ops2, err := AssembleStringWithVersion(dis, 8)
	require.NoError(t, err, "disassembly with mixed frame slot types could not be reassembled:\n%s", dis)
	require.Equal(t, ops.Program, ops2.Program)
}
...
pushint 1`
	...
	ops2, err := AssembleStringWithVersion(dis, 8)
	require.NoError(t, err, "disassembly with mixed scratch types could not be reassembled:\n%s", dis)
	require.Equal(t, ops.Program, ops2.Program)
}
