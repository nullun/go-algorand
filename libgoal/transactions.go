...
// SignTransactionWithWalletAsSponsor signs a transaction as a fee sponsor using the
// FeeSponsor domain-separated signature. The sponsor signs with their own key.
func (c *Client) SignTransactionWithWalletAsSponsor(walletHandle, pw []byte, sponsorAddr string, stx transactions.SignedTxn) (stxn transactions.SignedTxn, err error) {
	...
	// Sign with sponsor's own key (empty PublicKey means infer from sponsor address)
	resp, err := kmd.SponsorSignTransaction(walletHandle, pw, crypto.PublicKey{}, sponsorAddr, stx.Txn)
	if err != nil {
		return
	}
	...
	copy(stxn.Ssig.Sig[:], resp.Signature)
	return
}

// SignTransactionWithWalletAsSponsorAndSigner signs a transaction as a fee sponsor
// using a different signing key (for rekeyed sponsor accounts).
func (c *Client) SignTransactionWithWalletAsSponsorAndSigner(walletHandle, pw []byte, sponsorAddr, signerAddr string, stx transactions.SignedTxn) (stxn transactions.SignedTxn, err error) {
	sponsor, err := basics.UnmarshalChecksumAddress(sponsorAddr)
	if err != nil {
		return
	}
	...
	resp, err := kmd.SponsorSignTransaction(walletHandle, pw, crypto.PublicKey(authAddr), sponsorAddr, stx.Txn)
	if err != nil {
		return
	}
...
// using the FeeSponsor domain-separated signature.
func (c *Client) MultisigSignTransactionAsSponsor(walletHandle, pw []byte, sponsorAddr string, signerAddr string, stx transactions.SignedTxn, partial crypto.MultisigSig) (stxn transactions.SignedTxn, err error) {
	...
	signerAddrParsed, err := basics.UnmarshalChecksumAddress(signerAddr)
	if err != nil {
		return
	}
	...
	resp, err := kmd.MultisigSponsorSignTransaction(walletHandle, pw, sponsorAddr, "", stx.Txn, crypto.PublicKey(signerAddrParsed), partial)
	if err != nil {
		return
	}
	...
	stxn = stx
	stxn.Ssig.Sponsor = sponsor
	err = protocol.Decode(resp.Multisig, &stxn.Ssig.Msig)
	return
}
